						...
						if (uuid != null) {
							if (code.equals("0")) {
								...
								sender.sendMessage(Main.toBase(Main.PREFIX + "§aDu hast erfolgreich den Spieler §6"
										+ name + "§4§l PERMANENT§r§a von diesem Netzwerk wegen §6Hacking§a gebannt!"));
							} else if (code.equals("1")) {
								banPlayer(uuid, name, sender.getName(), "Bugusing",
										System.currentTimeMillis() + 1000 * 60 * 60 * 24 * 5, false);
								sender.sendMessage(Main.toBase(Main.PREFIX + "§aDu hast erfolgreich den Spieler §6"
										+ name + "§4 ZEITLICH§r§a von diesem Netzwerk wegen §6Bugusing§a gebannt!"));
							} else if (code.equals("2")) {
								banPlayer(uuid, name, sender.getName(), "Teaming",
										System.currentTimeMillis() + 1000 * 60 * 60 * 24 * 5, false);
								sender.sendMessage(Main.toBase(Main.PREFIX + "§aDu hast erfolgreich den Spieler §6"
										+ name + "§4 ZEITLICH§r§a von diesem Netzwerk wegen §6Teaming§a gebannt!"));
								...
								banPlayer(uuid, name, sender.getName(), "Trolling",
										System.currentTimeMillis() + 1000 * 60 * 60 * 24 * 5, false);
								sender.sendMessage(Main.toBase(Main.PREFIX + "§aDu hast erfolgreich den Spieler §6"
										+ name + "§4 ZEITLICH§r§a von diesem Netzwerk wegen §6Trolling§a gebannt!"));
							} else {
								sender.sendMessage(Main
										.toBase(Main.PREFIX + "§cBitte benutze: §6/ban <Spieler> <Grundnummer>§c!"));
