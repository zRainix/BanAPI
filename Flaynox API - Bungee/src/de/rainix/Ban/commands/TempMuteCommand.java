	...
	public void execute(CommandSender sender, String[] args) {
		if (sender.hasPermission("mute")) {
			if (args.length >= 3) {
				...
				if (UUID != null) {
					...
					if (args[1].endsWith("min")) {
						...
						try {
							...
							time = time * 1000 * 60;
							TIME = TIME + time;
							Main.muteUtil.tempmutePlayer(UUID, sender.getName(), CAUSE, TIME, "", ip, false);
							sender.sendMessage(Main.PREFIX + "§aDu hast erfolgreich den Spieler §6" + name
									+ " §awegen §6" + CAUSE + "§a für §6" + t[0] + " Minute(n) §agemutet!");
						...
						try {
							...
							Main.muteUtil.tempmutePlayer(UUID, sender.getName(), CAUSE, TIME, "", ip, false);
							sender.sendMessage(Main.PREFIX + "§aDu hast erfolgreich den Spieler §6" + name
									+ " §awegen §6" + CAUSE + "§a für §6" + t[0] + " Stunde(n) §agemutet!");
						} catch (NumberFormatException e) {
						}
						...
						try {
							...
							Main.muteUtil.tempmutePlayer(UUID, sender.getName(), CAUSE, TIME, "", ip, false);
							sender.sendMessage(Main.PREFIX + "§aDu hast erfolgreich den Spieler §6" + name
									+ " §awegen §6" + CAUSE + "§a für §6" + t[0] + " Tag(e) §agemutet!");
						} catch (NumberFormatException e) {
						}
						...
						try {
							...
							Main.muteUtil.tempmutePlayer(UUID, sender.getName(), CAUSE, TIME, "", ip, false);
							sender.sendMessage(Main.PREFIX + "§aDu hast erfolgreich den Spieler §6" + name
									+ " §awegen §6" + CAUSE + "§a für §6" + t[0] + " Woche(n) §agemutet!");
						} catch (NumberFormatException e) {
						}
						...
						try {
							...
							Main.muteUtil.tempmutePlayer(UUID, sender.getName(), CAUSE, TIME, "", ip, false);
							sender.sendMessage(Main.PREFIX + "§aDu hast erfolgreich den Spieler §6" + name
									+ " §awegen §6" + CAUSE + "§a für §6" + t[0] + " Monat(e) §agemutet!");
						} catch (NumberFormatException e) {
						}
					} else {
					...
					try {
						...
						sender.sendMessage("   §7Gemutet bis: §6" + time1 + " §8um §6" + time2 + " Uhr§8!");
				...
				sender.sendMessage(Main.PREFIX + "§cBitte benutze: §6/tempmute <Spieler> <Zeit>+Zeitangabe <Grund>§c!");
			}
		} else {
